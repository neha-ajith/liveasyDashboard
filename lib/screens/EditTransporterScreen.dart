...
class EditTransporterDetails extends StatelessWidget {
  ...
  EditTransporterDetails(
      {required this.dataList, required this.transporterDetails});
  ...
  Widget build(BuildContext context) {
    TextEditingController name =
        TextEditingController(text: transporterDetails.transporterName);
    transporterController.updateTransporterName(
        transporterDetails.transporterName == null
            ? ""
            : transporterDetails.transporterName!);

    TextEditingController contact =
        TextEditingController(text: transporterDetails.phoneNo);

    TextEditingController location =
        TextEditingController(text: transporterDetails.transporterLocation);
    transporterController.updateTransporterLocation(
        transporterDetails.transporterLocation == null
            ? ""
            : transporterDetails.transporterLocation!);

    TextEditingController companyName =
        TextEditingController(text: transporterDetails.companyName);
    transporterController.updateTransporterCompanyName(
        transporterDetails.companyName == null
            ? ""
            : transporterDetails.companyName!);

    transporterController.updateOnTransporterApproval(
        transporterDetails.transporterApproved! ? 1 : 2);
    transporterController.updateCompanyProofApproval(
        transporterDetails.companyApproved! ? 1 : 2);
                  ...
                  Obx(() {
                    return DocumentImageLayout(
                        ...
                        panApproved: transporterController
                            .identityProofApprovalStatus.value,
                        aadhar1Approved: transporterController
                            .addressProofFrontApprovalStatus.value,
                        aadhar2Approved: transporterController
                            .addressProofBacktApprovalStatus.value);
                  }),
                  ...
                  Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    TextButton(
                        ...
                        onPressed: () async {
                          ...
                          detailsForUpdate['transporterLocation'] =
                              location.text;
                          detailsForUpdate['companyName'] = companyName.text;
                          transporterController
                                      .transporterApprovalStatus.value ==
                                  1
                          ...
                          var statusCode = await updateTransporterStatus(
                              detailsForUpdate: detailsForUpdate,
                              userId: transporterDetails.transporterId);
                          if (statusCode == 200) {
                            showDialog(
                              context: context,
